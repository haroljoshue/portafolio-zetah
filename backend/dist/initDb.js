...
async function initDb() {
    ...
    try {
        ...
        const checkDbRes = await sysClient.query(`SELECT 1 FROM pg_database WHERE datname = $1`, [targetDb]);
    ...
    try {
        ...
        await client.query(`INSERT INTO users (username, password_hash) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [adminUser, passwordHash]);
        ...
        await client.query(`INSERT INTO profile (id, name, title, bio, avatar_url, cover_url, email, phone, instagram_url, facebook_url, whatsapp_url, slogan) 
       ...
       ON CONFLICT (id) DO UPDATE SET 
        ...
        // Seed initial notice
        await client.query(`INSERT INTO notices (title, content, is_active) VALUES ($1, $2, $3)`, ['¡Bienvenidos a mi nuevo portafolio!', 'Hola a todos, he lanzado este nuevo espacio para compartir mis producciones de video, trabajos de fotografía y proyectos de software. ¡Siéntete libre de contactarme!', true]);
        // Seed initial publication
        await client.query(`INSERT INTO publications (title, content, image_url) VALUES ($1, $2, $3)`, [
        ...
        try {
            ...
            if (fs_1.default.existsSync(datosPath)) {
                console.log('Loading actual videos and photos from zetah-gallery...');
                const datosContent = fs_1.default.readFileSync(datosPath, 'utf8');
                ...
                if (datos.videos && Array.isArray(datos.videos)) {
                    ...
                    for (let i = 0; i < datos.videos.length; i++) {
                        ...
                        await client.query(`INSERT INTO videos (title, original_url, youtube_id, embed_url, thumbnail_url, description, published_at) 
               VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING`, [title, originalUrl, youtubeId, embedUrl, thumbnailUrl, description, publishedAt]);
                    }
                }
                // Sembrar fotos
                if (datos.imagenes && Array.isArray(datos.imagenes)) {
                    ...
                    for (let i = 0; i < datos.imagenes.length; i++) {
                        ...
                        if (driveId) {
                            ...
                            await client.query(`INSERT INTO photos (title, original_url, drive_id, direct_url, description, registered_at) 
                 VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT DO NOTHING`, [title, originalUrl, driveId, directUrl, description, registeredAt]);
                        }
                    }
                }
            }
